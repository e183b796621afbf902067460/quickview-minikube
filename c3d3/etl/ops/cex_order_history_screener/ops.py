...
def extract_from_c3vault(context) -> List[dict]:
    query = '''
        ...
        WHERE
            h_exchanges.h_exchange_name LIKE '%%cex_order_history_screener%%'
        ORDER BY
            h_exchanges.h_exchange_name
    '''
...
def load_to_dwh(context, df: List[list]) -> None:
    ...
    concat_df.to_sql(name='pit_big_table_all_orders', con=context.resources.dwh.get_engine(), if_exists='append', index=False)
